...
class LWANet_Aggregation(nn.Module):  # base on PSMNet basic
    def __init__(self, input_planes=8, planes=16, maxdisp=192,  conv_3d_types1 = "P3D", conv_3d_types2 = "P3D", activation_types2 = "ELU"):
        ...
        self.pre_3D = nn.Sequential(
                            convbn_3d(input_planes, planes, 3, 1, 1, conv_3d_types =  "P3D"),
                            activation_function(types = "ELU"),
                            convbn_3d(planes, planes, 3, 2, 1, conv_3d_types =  "P3D"),
                            activation_function(types = "ELU")
                                    )

        self.middle_3D = nn.Sequential(

                        convbn_3d(planes, planes*2, 3, 1, 1,  conv_3d_types =  "P3D"),
                        activation_function(types = "ELU"),
                        convbn_3d(planes*2, planes*4, 3, 1, 1, conv_3d_types =  "P3D"),
                        activation_function(types = "ELU"),
                        convbn_3d(planes * 4, planes * 4, 3, 1, 1, conv_3d_types="P3D"),
                        activation_function(types="ELU"),
                        convbn_3d(planes * 4, planes * 2, 3, 1, 1, conv_3d_types="P3D"),
                        activation_function(types="ELU"),
                        convTranspose3d(planes * 2, planes * 2, kernel_size=3, stride=2, conv_3d_types="P3D"),
                        activation_function(types="ELU")
                                    )

        self.post_3D = nn.Sequential(
                                convbn_3d(planes*2, planes, 3, 1, 1, conv_3d_types =  "P3D"),
                                activation_function(types = "ELU"),
                                conv_3d(planes, 1, kernel_size=3, pad=1, stride=1, conv_3d_types =  "P3D")
                                    )
